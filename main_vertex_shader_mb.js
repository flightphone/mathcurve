...
let nx = 1000;  //widthSegments
let ny = 200;   //heightSegments
let nbx = 2;    //width distinct areas for one object
...
const uniforms = {
    ...
    u_light: { value: new THREE.Vector3(-1, 0, 0) },
    u_nx: { value: nx },
    u_ny: { value: ny },
    u_shape: { value: 0.0 }
};
...
void main() {
    ...
    vec3 light = normalize(vec3(0., 0., -1.));//normalize(u_light);//normalize(light2);
    vec3 col = vec3(.2, .2,  1.);
    float difu = abs(dot(nor, light));
    vec3 R = reflect (light, nor);
    float specular  = pow(max((dot(R, rd)), 0.), 20.);
    
    col = col*(col*clamp(difu, 0., 1.0) + 0.) + vec3(.5)*specular*specular;
    gl_FragColor = vec4(col , 1. );
}
`;
